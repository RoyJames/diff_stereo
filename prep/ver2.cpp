...
public:
	...
	virtual void f(la_vector<float> &fx, const la_vector<float> &x)
	{
		...
		for (int i = 0; i < fx.length; i++)
		{
			...
			fx.v[i] =
				...
				coef[i * 4 + 0] * x.v[j * 2 + 0] + coef[i * 4 + 1] * x.v[j * 2 + 1] +
				coef[i * 4 + 3] / (coef[i * 4 + 2] - beta*x.v[x.length - 1]);
		}
	}
	...
	virtual void j(la_vector<float> &jx, const la_vector<float> &x)
	{
		...
		for (int i = 0; i < viewid.size(); i++)
		{
			...
			jx.v[i * x.length + j * 2 + 0] += coef[i * 4 + 0];
			jx.v[i * x.length + j * 2 + 1] += coef[i * 4 + 1];
...
{
	vector2f uv;
	uv.x = 0.25f;// 0.687500f;// 0.00195312f;
	uv.y = 0;// -0.00195312f;
	...
	std::vector<float> pixel0;
	{
		...
		{
			vector2f scr;
			uv2screen(scr, solver_data[i].uv, width, height);
			vector3f c;
			lerp_img(c.v, scr, img, width, height, 3);
			...
			for (int k = 0; k < solver_data.size(); k++)
			{
				...
				if (b_svd.size() > nsamples / 2)
				{
					...
					if (abs(gI.x) < 10000 && abs(gI.y) < 10000 && abs(gI.z) < 10000)
					{
						...
						sd.ref.push_back(gI.z - pixel0[k]);
	...
	for (int k = 0; k < solver_data.size(); k++)
	{
		...
		if (sd.b_valid)
		{
			...
			for (int j = 0; j < 40; j++)
			{
				float z = z0 + (z1 - z0)*j / 40;
				float invz = 1.0f / (1.0f - z / FOCAL_LEN);

				la_matrix<float> A(sd.ref.size(), 2);
				for (int i = 0; i < A.row; i++)
				{
					A.m[i + 0 * A.row] = sd.func.coef[i * 4 + 0];
					A.m[i + 1 * A.row] = sd.func.coef[i * 4 + 1];
				}
				la_vector<float> b(sd.ref), ATb;
				for (int i = 0; i < b.length; i++)
					b.v[i] -= invz*sd.func.coef[i * 4 + 3];
				...
				for (int i = 0; i < b.length; i++)
				{
					float v = sd.func.coef[i * 4 + 0] * x.v[0] +
							sd.func.coef[i * 4 + 1] * x.v[1] 
							- b.v[i];
					err += v*v;
				}

				printf_s("#%02d (%f %f) %f %f %f\n", 
					j, x.v[0], x.v[1], atan2f(x.v[1], x.v[0]), z, err);
			}

			{
				la_matrix<float> A(sd.ref.size(), 3);
				for (int i = 0; i < A.row; i++)
				{
					A.m[i + 0 * A.row] = sd.func.coef[i * 4 + 0];
					A.m[i + 1 * A.row] = sd.func.coef[i * 4 + 1];
					A.m[i + 2 * A.row] = sd.func.coef[i * 4 + 3];
				}
				la_vector<float> b(sd.ref), ATb;
			...
			la_vector<float> x(2 * NUM_IMG_VIEW + 1);
			x.clear();
